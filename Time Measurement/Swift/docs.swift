func help() {
	write(FH.standardOutput,clean("""
		 ...
		 使い方:
		  measure-swift [options] [command] [arg1] [arg2]…
		  measure-swift -multiple [options] "[command1]" "[command2]"…
		  ...
		  オプション
		   ...
		   -m,-multiple [string?]
		    ...
		    • spawn,parallel
		     シェルの同時起動により並列実行します
		    • operation
		     OperationQueue を用いて並列実行します
		    • dispatch
		     DispatchQueue を用いて並列実行します
